...
resource "aws_vpc" "vpc" {
  ...
  tags = {
    Name        = var.create_prefix_for_resources ? "${local.prefix}_${var.vpc[0].name}" : "${var.vpc[0].name}"
    Project     = var.project
    Environment = var.env
  }
}
...
resource "aws_subnet" "subnet-3" {
  vpc_id                  = aws_vpc.vpc.id
  cidr_block              = var.subnet_private_2[0].cidr_block
  availability_zone       = var.subnet_private_2[0].availability_zone
...
resource "aws_network_interface" "network_interface" {
  ...
  tags = {
    Name        = var.create_prefix_for_resources ? "${local.prefix}_${var.network_interface[0].name}" : "${var.network_interface[0].name}"
    Project     = var.project
    Environment = var.env
  }
}
